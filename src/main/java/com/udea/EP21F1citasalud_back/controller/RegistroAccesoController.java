...
public class RegistroAccesoController {
    ...
    @Operation(summary = "Obtener todos los registros de acceso", description = "Retorna todos los registros de acceso del sistema")
    @ApiResponse(responseCode = "200", description = "Operación exitosa",
            content = @Content(mediaType = "application/json",
                    schema = @Schema(implementation = RegistroAcceso.class)))
    public ResponseEntity<List<RegistroAccesoDTO>> getAll() {
        List<RegistroAccesoDTO> dtos = registroAccesoRepository.findAll().stream().map(this::toDTO).collect(Collectors.toList());
